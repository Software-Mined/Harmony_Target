...
{
    public partial class KPIReport : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void KPIReport_DataSourceDemanded(object sender, EventArgs e)
        {
            ...
            //Prev Month
            int PrevMonth = Convert.ToInt32(ProductionGlobal.ProductionGlobal.ProdMonthAsString(Convert.ToDateTime(Parameters["Prodmonth"].Value))) - 1;

            //KPI
            MWDataManager.clsDataAccess _dbMandata = new MWDataManager.clsDataAccess();
            _dbMandata.ConnectionString = _Connection;            
            _dbMandata.SqlStatement = " exec KPI_Dashboard '" + ProductionGlobal.ProductionGlobal.ProdMonthAsString(Convert.ToDateTime(Parameters["Prodmonth"].Value)) + "','" + PrevMonth + "','" + System.DateTime.Today.ToShortDateString() + "', '" + SysSettings.Banner + "' ";                                            
            ...
            ///Dev SubReport
            KPIReportDev _KPIDev = new KPIReportDev();
            _KPIDev._Connection = _Connection;           
            _KPIDev._Prodmonth = ProductionGlobal.ProductionGlobal.ProdMonthAsString(Convert.ToDateTime(Parameters["Prodmonth"].Value));            
            subReportKPIDev.ReportSource = _KPIDev;
