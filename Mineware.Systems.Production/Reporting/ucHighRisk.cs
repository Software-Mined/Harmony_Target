...
{
    public partial class ucHighRisk : BaseUserControl
    {
        ...
        public void LoadData()
        {
            ...
            if (editGroupBy.EditValue.ToString() == "Workplace")
            {
                reportFileName = "HighRisk.frx";
                tableName = "6Shift";
                sql = @"EXEC [dbo].[sp_Report_HighRisk] '" + ProductionGlobal.ProductionGlobal.ProdMonthAsString(Convert.ToDateTime(cbxBookingProdMonth.EditValue.ToString())) + @"', '" + cbxBookingMO.EditValue.ToString() + @"' ";
            }
            if (editGroupBy.EditValue.ToString() == "Department")
            {
        ...
        private void editGroupBy_EditValueChanged(object sender, EventArgs e)
        {
            if (_FirstLoad == "N")
                LoadData();
        }
