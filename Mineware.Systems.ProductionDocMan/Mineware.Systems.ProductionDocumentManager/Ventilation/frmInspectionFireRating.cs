...
namespace Mineware.Systems.DocumentManager.Ventilation
{
    public partial class frmInspectionFireRating : DevExpress.XtraEditors.XtraForm
    {
        ...
        //string repDir = @"\\10.148.225.119\Mineware\Images\VentilationInspections\FireInspections\DocumentFiles";    //Path to store files
        //string repImgDir = @"\\10.148.225.119\Mineware\Images\VentilationInspections\FireInspections\Images";  //Path to store Images
        //string ActionsImgDir = @"\\10.148.225.119\Mineware\Images\VentilationInspections\FireInspections\ActionImages";

        string repDir = ProductionGlobal.ProductionGlobalTSysSettings._RepDir + @"\VentilationInspections\FireInspections\DocumentFiles";    //Path to store files
        string repImgDir = ProductionGlobal.ProductionGlobalTSysSettings._RepDir + @"\VentilationInspections\FireInspections\Images";  //Path to store Images
        ...
        private void gvFireRating_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (e.Column.FieldName.ToString() == "YesNo")
            {
                ...
                if (gvFireRating.GetRowCellValue(e.RowHandle, gvFireRating.Columns["Hazard"]).ToString() != string.Empty)
                {
                    ...
                    ActFrm.txtProdMonth.EditValue = procs.ProdMonthAsString(Convert.ToDateTime(tbProdMonth.EditValue.ToString()));
        ...
        private void btnEditMinAct_Click(object sender, EventArgs e)
        {
            ...
            frmVentilationActionCapture ActFrm = new frmVentilationActionCapture();
            ActFrm.txtProdMonth.EditValue = procs.ProdMonthAsString(Convert.ToDateTime(tbProdMonth.EditValue.ToString())); //String.Format("{0:yyyy-MM-dd}", tbProdMonth.EditValue.ToString());
            ActFrm.txtSection.EditValue = tbSection.EditValue;
